...
turtle.shape("CharacterRight.gif")
turtle.register_shape("CharacterLeft.gif")
turtle.shape("CharacterLeft.gif")
#princess = Turtle(shape='CharecterRight.gif')
turtle.penup()
turtle.setposition(100, -300)

def boundary_check():
    # boundary check
    if not -450 < turtle.xcor() < 450 or not -450 < turtle.ycor() < 450:
        turtle.right(180)

    if iscollision(turtle, goal):
        goal.hideturtle()
